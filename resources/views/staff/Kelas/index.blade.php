        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                                        ...
                                        <div class="mt-2">
                                            <form action="{{ route('staff.kelas.store') }}" method="POST">
                                                ...
                                                <div class="mb-4">
                                                    ...
                                                    <input type="text" name="nama_kelas" id="nama_kelas" class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" required>
                                                </div>
                                                <div class="mb-4">
                                                    <label for="user_name" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Nama Siswa</label>
                                    ...
                                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                        ...
                                        <div class="mt-2">
                                            <form :action="'/staff/kelas/' + currentKelas.id_kelas" method="POST">
                                                ...
                                                <div class="mb-4">
                                                    <label for="nama_kelas" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Nama Kelas</label>
                                                    <input type="text" name="nama_kelas" id="nama_kelas" x-model="currentKelas.nama_kelas" class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" required>
                                                </div>
                                                <div class="mb-4">
                                                    <label for="user_name_edit" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Nama Siswa</label>
                                                    <input type="text" id="user_name_edit" class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" list="user-options-edit" x-model="currentKelas.user.name" required>
                                                    <datalist id="user-options-edit" class="bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md shadow-lg">
                                                        @foreach ($users as $user)
                                                            <option data-id="{{ $user->id }}" value="{{ $user->name }}" class="p-2 hover:bg-gray-100 dark:hover:bg-gray-600"></option>
                                                        @endforeach
                                                    </datalist>
                                                    <input type="hidden" name="user_id" id="user_id_edit" x-model="currentKelas.user_id">
                                                </div>
                                                ...
                                                <div class="flex justify-end">
                                                    <button type="button" @click="openEdit = false" class="mr-2 inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-gray-700 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600">Cancel</button>
                    ...
                    <div x-show="openAddMember" class="fixed z-10 inset-0 flex items-center justify-center overflow-y-auto">
                        <div class="fixed inset-0 transition-opacity" aria-hidden="true">
                            <div class="absolute inset-0 bg-gray-500 opacity-75"></div>
                        </div>
                            ...
                            <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                <div class="sm:flex sm:items-start">
                                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                        ...
                                        <div class="mt-2">
                                            <form :action="'/kelas/' + currentKelas.id_kelas + '/add-member'" method="POST">
                                                @csrf
                                                <div class="mb-4">
                                                    <label for="user_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Pilih Siswa</label>
                                                    <input type="text" name="user_id" id="user_id" class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" required list="user-options">
                                                    <datalist id="user-options">
                                                ...
                                                <div class="flex justify-end">
                                                    <button type="button" @click="openAddMember = false" class="mr-2 inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-gray-700 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600">Cancel</button>
                                                    <button type="submit" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-green-600 hover:bg-green-700">Tambah</button>
                                                </div>
                                            </form>
                                        </div>
                                        <div class="mt-4">
                                            <h4 class="text-md leading-6 font-medium text-gray-900 dark:text-gray-100">Daftar Anggota Kelas</h4>
                                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                                ...
                                                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                                    <template x-for="(user, index) in currentKelas.users" :key="user.id">
                                                        <tr>
                                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white" x-text="index + 1"></td>
                                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300" x-text="user.name"></td>
                                                        </tr>
                                                    </template>
                                                </tbody>
                                            </table>
                                ...
                                @foreach ($kelas as $kelasItem)
                                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                        ...
                                        <td class="flex items-center px-6 py-4">
                                            <button @click="openEdit = true; currentKelas = {{ $kelasItem }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded flex items-center">
                                                <i class="fas fa-pencil-alt mr-2"></i>Edit
                                            </button>
                                            <button @click="openDelete = true; currentKelas = {{ $kelasItem }}" class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded ms-3 flex items-center">
                                                <i class="fas fa-trash-alt mr-2"></i>Remove
                                            </button>
                                            <button @click="openAddMember = true; currentKelas = {{ $kelasItem }}" class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded ms-3 flex items-center">
